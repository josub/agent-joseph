---
title: Leadodex (contact-scanner) — Go-To-Market Plan
status: durable deliverable
brand: Leadodex
repo: github.com/josub/contact-scanner (separate repo, not this brain)
source: 2026-06-28 contact-scanner GTM session (decided with Joseph)
created: 2026-06-28
promoted_to_projects: 2026-06-29 (by sleep, from memory/working/)
---

# Leadodex — GTM plan

The **full** go-to-market plan for Business #1 (Leadodex, the contact/lead scanner).
Decided with Joseph 2026-06-28. By Joseph's instruction the full plan lives here in the
brain, **not** in the code repo; only the referral section is also mirrored to the repo at
`contact-scanner/docs/REFERRAL-PROGRAM.md`. Several points below are user-confirmed
decisions (pricing reality, referral mechanics), not proposals.

Grounded in the repo (README/ROADMAP/PROGRESS — camera-first mobile app: Claude Vision
...
- Statement: *For B2B sales and field-marketing teams who work events, Leadodex turns any
  badge or business card into an enriched, CRM-ready lead in seconds, no rented hardware, no
  manual entry, no leads lost. Unlike basic scanners and OCR card apps, every scan arrives
  qualified and routed while the lead is still warm.*
...
- Three shipped proof pillars: auto-enrichment → qualified lead; one-tap HubSpot + real-time
  Slack → in CRM before you leave the booth; flat per-seat price < renting one scanner for
  one show.
...
## Referral program (two tracks; full spec also in repo docs/REFERRAL-PROGRAM.md)
...
prompt). Optional 2x month for annual referees. Anti-abuse: grant only after first charge
